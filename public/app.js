...
if (tg) {
  try {
    ...
    console.log('Telegram SDK Init:', e);
  }
}
  ...
  fr: {
    ...
    policy: {
      ...
      sections: [
        ...
        { title: '3. Respect des Délais', text: 'Nous nous engageons sur un calendrier clair (4 à 10 jours selon l\'envergure du projet). Un suivi régulier vous est partagé sur WhatsApp ou Telegram.' },
    ...
    order: {
      ...
      successMsg: 'Merci <strong>{name}</strong> ! Votre commande pour <strong>{service}</strong> a bien été enregistrée. Notre équipe vous contactera sous 2h.',
      close: 'Fermer',
      ...
      services: [
        {
          ...
          price: '60 000 FCFA (au lieu de 120k, ~ $100)',
          delay: '4-6 jours'
        },
        {
          id: 'ecommerce',
  ...
  en: {
    ...
    policy: {
      ...
      sections: [
        ...
        { title: '3. Punctual Delivery', text: 'We commit to realistic deadlines (4 to 10 days). Regular progress checkpoints are shared on WhatsApp or Telegram.' },
    ...
    order: {
      ...
      successMsg: 'Thank you <strong>{name}</strong>! Your order for <strong>{service}</strong> has been logged. Our team will contact you within 2 hours.',
      close: 'Close',
...
function openOrderModal(serviceId) {
  ...
  modalBody.innerHTML = `
    <div class="modal-header">
      ...
      <div class="modal-sub-price">${s.price}</div>
    </div>
