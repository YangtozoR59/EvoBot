...
app.post('/api/send-order', async (req, res) => {
  try {
    ...
    const messageHtml =
      `🚀 <b>NOUVELLE COMMANDE REÇUE — Evodevs Team (-50%)</b>\n\n` +
      ...
      `⚡ <b>Urgent :</b> ${urgent ? '🔴 <b>OUI (Livraison Express)</b>' : '⚪ Standard'}\n\n` +
...
// ==========================================
// 3. CATALOGUE DES SERVICES (PRIX DIVISÉS PAR 2)
// ==========================================
const servicesData = {
  web: {
    ...
    delay: { fr: '4 à 6 jours ouvrés', en: '4 to 6 business days' },
    desc: {
      fr: 'Une présence en ligne propre, moderne et ultra-rapide optimisée pour les smartphones et réseaux 3G. Idéal pour indépendants et commerçants.',
    ...
    features: {
      fr: [
        '1 à 3 pages responsives modernes',
        'Bouton WhatsApp direct & appel en 1 clic',
        'Nom de domaine & hébergement 1 an offert',
        'Référencement Google SEO de base',
        'Garantie 14 jours incluse'
      ],
      en: [
        '1 to 3 fast responsive pages',
        'Direct WhatsApp click-to-chat & call button',
  ...
  ecommerce: {
    ...
    delay: { fr: '7 à 10 jours ouvrés', en: '7 to 10 business days' },
    desc: {
      fr: 'Site d\'entreprise complet avec catalogue interactif de vos produits ou services, formulaires de devis et référencement Google Maps.',
    ...
    features: {
      fr: [
        ...
        'Formulaires de réservation / devis',
        'Formation vidéo pour modifier vos textes'
      ],
      en: [
        'Up to 6 custom pages',
        'Interactive catalog & dynamic filters',
  ...
  telegram_app: {
    title: { fr: '🛍️ Telegram Mini App E-Commerce', en: '🛍️ Telegram Mini App E-Commerce' },
    price: { fr: '175 000 – 325 000 FCFA (~ $285 – $530)', en: '$285 – $530 (~ 175k – 325k FCFA)' },
    delay: { fr: '7 à 12 jours ouvrés', en: '7 to 12 business days' },
    desc: {
      fr: 'Votre boutique complète nativement intégrée dans Telegram ! Vos clients parcourent les produits, ajoutent au panier et commandent sans quitter l\'application.',
      en: 'Your full online shop embedded inside Telegram! Customers browse products, manage cart, and order without leaving Telegram.'
    },
    features: {
      fr: [
        'Interface Telegram WebApp native & fluide',
        'Panier d\'achat dynamique',
        'Réception instantanée des commandes',
        'Gestion simple de vos articles',
        'Déploiement Cloud sécurisé inclus'
      ],
      en: [
  ...
  design: {
    title: { fr: '🎨 Identité Visuelle & Logo', en: '🎨 Visual Identity & Logo' },
    price: { fr: '30 000 – 60 000 FCFA (~ $50 – $100)', en: '$50 – $100 (~ 30k – 60k FCFA)' },
    delay: { fr: '3 à 5 jours ouvrés', en: '3 to 5 business days' },
    desc: {
      fr: 'Créez une marque forte qui inspire immédiatement confiance avec un logo vectoriel haute définition et une charte graphique professionnelle.',
    ...
    features: {
      fr: [
        ...
        'Bannières réseaux sociaux adaptées',
  ...
  bot: {
    ...
    delay: { fr: '3 à 6 jours ouvrés', en: '3 to 6 business days' },
    desc: {
      fr: 'Automatisez vos ventes et votre support client 24h/24 grâce à un bot interactif connecté à votre activité.',
    ...
    features: {
      fr: [
        ...
        'Gestion des réponses automatiques FAQ',
        'Hébergement Cloud sécurisé',
        'Prise en main simple et rapide'
      ],
      en: [
        'Custom interactive commands & menus',
  ...
  maintenance: {
    title: { fr: '🛡️ Maintenance & Sérénité', en: '🛡️ Maintenance & Peace of Mind' },
    price: { fr: '7 500 FCFA / mois (ou 60 000 FCFA / an, ~ $12/m)', en: '$12 / month (or $100 / year)' },
    delay: { fr: 'Support continu', en: 'Continuous support' },
    desc: {
      fr: 'Déléguez toute la technique : hébergement, sauvegardes régulières, mises à jour de sécurité et support réactif sur WhatsApp.',
...
const userOrderSession = new Map();
...
const getMainMenu = (ctx) => {
  ...
  buttons.push([
    Markup.button.callback(isFr ? '💬 Contacter un Développeur' : '💬 Talk to Developer', 'cmd_contact'),
    Markup.button.url('📲 WhatsApp Direct', 'https://wa.me/237698448024')
  ]);
...
// Menu de sélection des services détaillés
const getServicesMenu = (ctx) => {
  const lang = getLang(ctx);
  const isFr = lang === 'fr';

  return Markup.inlineKeyboard([
    [Markup.button.callback(isFr ? '🌐 Site Vitrine (60 000 F)' : '🌐 Showcase Site ($100)', 'detail_web')],
    [Markup.button.callback(isFr ? '🚀 Site Pro & Catalogue (125k F)' : '🚀 Pro Business Site ($200)', 'detail_ecommerce')],
    [Markup.button.callback(isFr ? '🛍️ Mini App Telegram (175k F)' : '🛍️ Telegram Mini App ($285)', 'detail_telegram_app')],
    [Markup.button.callback(isFr ? '🎨 Identité & Logo (30 000 F)' : '🎨 Logo & Identity ($50)', 'detail_design')],
    [Markup.button.callback(isFr ? '🤖 Bot Telegram (50 000 F)' : '🤖 Telegram Bot ($80)', 'detail_bot')],
    [Markup.button.callback(isFr ? '🛡️ Maintenance (7 500 F/m)' : '🛡️ Maintenance ($12/m)', 'detail_maintenance')],
    [
      ...(WEBAPP_URL ? [Markup.button.webApp(isFr ? '🚀 Ouvrir la Mini App' : '🚀 Open Mini App', WEBAPP_URL)] : []),
      Markup.button.callback(isFr ? '⬅️ Menu Principal' : '⬅️ Main Menu', 'main_menu')
    ]
  ]);
};
...
function renderServiceDetail(ctx, key) {
  ...
  const text =
    `<b>${s.title[lang]}</b>\n\n` +
    `📖 <b>Description :</b>\n${s.desc[lang]}\n\n` +
    `💰 <b>Tarif Spécial (-50%) :</b> <code>${s.price[lang]}</code>\n` +
    `⏱ <b>Délai estimé :</b> ${s.delay[lang]}\n\n` +
    `🎁 <b>Ce qui est inclus :</b>\n${featuresText}\n\n` +
    `<i>Comment souhaitez-vous procéder ?</i>`;
  ...
  const buttons = [
    ...
    [
      Markup.button.url(isFr ? '📲 Échanger sur WhatsApp' : '📲 Chat on WhatsApp', waUrl),
      ...(WEBAPP_URL ? [Markup.button.webApp(isFr ? '🚀 Mini App' : '🚀 Mini App', WEBAPP_URL)] : [])
    ],
    [Markup.button.callback(isFr ? '⬅️ Retour aux Services' : '⬅️ Back to Services', 'cmd_services')]
  ];
...
bot.start((ctx) => {
  ...
  const welcomeText = isFr
    ? `👋 Bonjour <b>${escapeHtml(name)}</b> !\n\n` +
      ...
      `🔥 <b>OFFRE SPÉCIALE EN COURS :</b> -50% sur l'ensemble de nos créations de sites web, boutiques Telegram Mini Apps et identités graphiques !\n\n` +
      `Explorez nos offres, découvrez les détails complets de chaque prestation ou lancez votre commande en direct :`
    : `👋 Hello <b>${escapeHtml(name)}</b> !\n\n` +
      ...
      `Explore our packages, view full service details, or order directly:`;
...
bot.command('services', (ctx) => {
  ...
  const text = isFr
    ? `💼 <b>Catalogue des Services Evodevs (-50%)</b>\n\n` +
      `Cliquez sur un service ci-dessous pour voir la <b>description détaillée, les fonctionnalités incluses, le délai et le tarif</b> :`
    : `💼 <b>Evodevs Services Catalog (-50% OFF)</b>\n\n` +
      `Tap any service below to view <b>full details, included deliverables, turnaround, and pricing</b>:`;
...
bot.command(['commander', 'order', 'devis'], (ctx) => {
  ...
  const text = isFr
    ? `📝 <b>Commander une Solution Evodevs (-50%)</b>\n\n` +
      `Sélectionnez le service que vous désirez commander ci-dessous :`
    : `📝 <b>Order an Evodevs Solution (-50% OFF)</b>\n\n` +
      `Select the service you would like to order below:`;
...
bot.command('contact', (ctx) => {
  ...
  const text = isFr
    ? `💬 <b>Points de Contact Directs — Evodevs Team</b>\n\n` +
      `📲 <b>WhatsApp :</b> +237 698 44 80 24\n` +
      `✈️ <b>Telegram Développeur :</b> @ItzCyd\n` +
      `📧 <b>E-mail :</b> evodevsteam.contact@gmail.com\n` +
      `📍 <b>Bureaux :</b> Ngaoundéré & Yaoundé, Cameroun\n\n` +
      `<i>Réponse humaine garantie sous 2h.</i>`
    : `💬 <b>Direct Contact Points — Evodevs Team</b>\n\n` +
      `📲 <b>WhatsApp:</b> +237 698 44 80 24\n` +
      `✈️ <b>Telegram Developer:</b> @ItzCyd\n` +
      `📧 <b>Email:</b> evodevsteam.contact@gmail.com\n` +
      `📍 <b>Offices:</b> Cameroon (WAT Timezone)\n\n` +
      `<i>We reply within 2 hours during daytime.</i>`;

  ctx.reply(text, {
    parse_mode: 'HTML',
    ...Markup.inlineKeyboard([
      [Markup.button.url('📲 WhatsApp Direct (+237 698 44 80 24)', 'https://wa.me/237698448024')],
      [Markup.button.callback(isFr ? '⬅️ Menu Principal' : '⬅️ Main Menu', 'main_menu')]
    ])
  });
});
...
bot.command('help', (ctx) => {
  ...
  const text = isFr
    ? `ℹ️ <b>Commandes disponibles :</b>\n\n` +
      ...
      `• /services — Explorer le catalogue avec fiches détaillées\n` +
      `• /commander — Passer commande d'un service\n` +
      `• /contact — Nos coordonnées directes\n` +
      `• /lang — Changer la langue (FR / EN)`
    : `ℹ️ <b>Available Commands:</b>\n\n` +
      `• /start — Main menu\n` +
      `• /services — Browse catalog with detailed views\n` +
...
bot.action('cmd_services', (ctx) => {
  ...
  const text = isFr
    ? `💼 <b>Catalogue des Services Evodevs (-50%)</b>\n\n` +
      `Cliquez sur un service ci-dessous pour voir la <b>fiche détaillée</b> :`
    : `💼 <b>Evodevs Services Catalog (-50% OFF)</b>\n\n` +
      `Tap any service below to view its <b>detailed view</b>:`;
...
bot.action('cmd_order_menu', (ctx) => {
  ...
  const text = isFr
    ...
    : `📝 <b>Choose the solution you want to order:</b>`;
...
['web', 'ecommerce', 'telegram_app', 'design', 'bot', 'maintenance'].forEach((key) => {
  ...
  bot.action(`start_order_${key}`, async (ctx) => {
    ...
    const notifAdmin =
      `🚀 <b>INTENTION DE COMMANDE DÉCLENCHÉE SUR TELEGRAM !</b>\n\n` +
      `👤 <b>Utilisateur :</b> ${escapeHtml(user.first_name || '')} ${escapeHtml(user.last_name || '')} (@${user.username || 'Sans @'})\n` +
      `🆔 <b>Telegram ID :</b> <code>${user.id}</code>\n` +
      `💼 <b>Service :</b> ${s.title[lang]}\n` +
      `💰 <b>Tarif :</b> ${s.price[lang]}\n` +
      `🕒 <i>Déclenché via bouton in-chat</i>`;
...
bot.action('lang_fr', (ctx) => {
  ...
  ctx.editMessageText(
    `👋 Bonjour <b>${escapeHtml(name)}</b> !\n\n` +
    `Bienvenue sur le bot officiel d'<b>Evodevs Team</b> 🚀\n\n` +
    `🔥 <b>OFFRE SPÉCIALE :</b> -50% sur tous nos services web !\n` +
...
bot.launch({ dropPendingUpdates: true })
  .then(() => console.log('🤖 Bot Telegram Evodevs v2.0 lancé avec succès !'))
  .catch((err) => {
    console.error('⚠️ Erreur démarrage bot Telegram :', err.message);
  });
